/* global Notification,localStorage,alert */
(function ($, wellClient) {
...
>>>>>>> 430a707db95939387acd398ec1c28d91270997c4
  }
  ...
  wellClient.ctrl.deliverMethod = function (id) {
    switch (id) {
      case 'well-make':
        return wellClient.ctrl.makeCall()
      case 'well-answer':
        return wellClient.ctrl.answerCall()
      case 'well-drop':
        return wellClient.ctrl.dropCall()
      case 'well-hold':
        return wellClient.ctrl.holdCall()
      case 'well-single':
        return wellClient.ctrl.singleCall()
      case 'well-consult':
        return wellClient.ctrl.consultCall()
      case 'well-cancel':
        return wellClient.ctrl.cancelCall()
      case 'well-transfer':
        return wellClient.ctrl.transferCall()
      case 'well-conference':
        return wellClient.ctrl.conferenceCall()
      case 'well-logout':
        return wellClient.ctrl.logout()
      case 'well-login':
  ...
  wellClient.ctrl.record = {
    main: function () {
      var $wellRecord = $('#well-record')
      if ($wellRecord.length === 0) {
        return
      }
      if ($wellRecord.html() === '正在录音中') {
        this.stopRecording()
      } else {
        this.startRecording()
      }
    ...
    stopRecordingDone: function () {
      $('#well-record')
        .attr('disabled', false)
        .removeClass('well-disabled well-record-on')
